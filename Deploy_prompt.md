...
5.  **GitHub Secrets**: Konfigurér workflowet til at bruge følgende 3 secrets:
    *   `FTP_SERVER`
    *   `FTP_USERNAME`
    *   `FTP_PASSWORD`

6.  **Verificering**: Tjek `package.json` for at sikre, at `npm run build` producerer en `dist/` mappe.
